...
function getPortfolioFromScores(scores: ScoreMap): FinalPortfolioKey {
  ...
  return gap >= 3 ? "crescita_controllata" : "crescita_lungo_periodo";
}
...
export default function Home() {
  ...
  const [purchase, setPurchase] = useState<PurchaseState>({ unlocked: false, email: "" });
  ...
  useEffect(() => {
    ...
    else setPurchase({ unlocked: false, email: user.email || "", selectedPortfolio: undefined });
  ...
  async function saveHoldingToDb(item: Holding) {
    ...
    const { error } = await supabase
      ...
      .upsert({
        ...
        holding_key: item.id,
        category: item.category,
        strumenti_name: item.strumentiName,
        isin: item.isin,
        amount: item.amount,
  ...
  async function loadHoldingsFromDb(currentUser: User) {
    ...
    if (data && data.length > 0) {
      const mapped: Holding[] = data.map((row: any) => ({
        ...
        strumentiName: row.strumenti_name,
        isin: row.isin,
  ...
  function resetAll() {
    ...
    if (user) {
      localStorage.removeItem(getPurchaseKey(user.id));
      localStorage.removeItem(getHoldingsKey(user.id));
      localStorage.removeItem(getRetakeStorageKey(user.id));
      Object.keys(portfolioMap).forEach((key) => {
        localStorage.removeItem(getChecklistStorageKey(user.id, key as FinalPortfolioKey));
        localStorage.removeItem(getPacStorageKey(user.id, key as FinalPortfolioKey));
      });
    }
    setAnswers(Array(questions.length).fill(-1));
    setCurrentQuestion(0);
    setPurchase({ unlocked: false, email: user?.email || "", selectedPortfolio: undefined });
    setHoldings([]);
    setChecklistState({});
  ...
  if (!user) {
    return (
      <main className="min-h-screen bg-slate-50 text-slate-900">
        <div className="mx-auto grid min-h-screen max-w-6xl gap-6 px-6 py-10 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm md:p-10">
            <p className="text-sm font-medium uppercase tracking-[0.2em] text-slate-500">Soldi Semplici</p>
            <h1 className="mt-3 text-4xl font-bold tracking-tight md:text-5xl">Accedi alla tua area personale</h1>
            <p className="mt-5 text-lg leading-8 text-slate-600">
              Registrati o accedi per salvare il tuo percorso, il tuo PAC mensile, la checklist operativa e i tuoi progressi.
            </p>

            <div className="mt-8 grid gap-4 md:grid-cols-3">
              <FeatureCard title="Modello personale" text="Il tuo profilo e il tuo percorso restano collegati al tuo account." />
              <FeatureCard title="Progressi salvati" text="Checklist, badge e storico mensile restano disponibili quando rientri." />
              <FeatureCard title="Prodotto serio" text="Da qui in poi l'app smette di essere una demo e diventa un servizio." />
            </div>
          </div>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
            ...
            <div className="mt-6 space-y-4">
              ...
              <button
                ...
                className="w-full rounded-xl bg-slate-900 px-6 py-3 font-medium text-white transition hover:bg-slate-800 disabled:opacity-50"
              >
                {authLoading ? "Attendi..." : authMode === "login" ? "Accedi" : "Crea account"}
              </button>

              <p className="text-xs leading-6 text-slate-500">
                Se stai creando un account, Supabase potrebbe chiederti di confermare la mail prima del primo accesso.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 md:px-8 md:py-8">
          ...
          <section className="space-y-6">
            <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm md:p-10">
              ...
              <h1 className="mt-3 max-w-3xl text-4xl font-bold tracking-tight md:text-5xl">
                Costruisci un metodo e mantienilo nel tempo
              </h1>
              <p className="mt-5 max-w-3xl text-lg leading-8 text-slate-600">
                L'app ti aiuta a costruire un metodo educativo basato su PAC mensile, continuita e consapevolezza. Non sostituisce una consulenza finanziaria personalizzata.
                Tenere i soldi fermi non e una scelta neutra: l'inflazione ne riduce il potere d'acquisto.
              </p>
              <p className="mt-4 max-w-3xl text-base leading-7 text-slate-600">
                Qui trovi un percorso semplice: questionario, modello assegnato, guida operativa,
                foglio strumenti e supporto al ribilanciamento annuale.
              </p>
              ...
              <div className="mt-8 grid gap-4 md:grid-cols-2">
                <InfoBox
                  ...
                  tone="green"
                  items={[
                    "Questionario guidato e assegnazione modello",
                    "Modello completo con spiegazioni chiare",
                    "Guida PAC e foglio strumenti con ISIN / esempi",
                    "Percorso semplice e adatto ai neofiti",
                  ]}
                />
                <InfoBox
                  title="Cosa NON offriamo"
                  tone="amber"
                  items={[
                    "Consulenza finanziaria personalizzata",
                    "Promesse di rendimento",
                    "Previsioni di mercato",
                    "Una risposta perfetta valida per tutti",
                  ]}
                />
              </div>

              <div className="mt-8 grid gap-4 lg:grid-cols-3">
                <FeatureCard title="1. Scopri il tuo profilo" text="8 domande semplici per capire il tuo approccio reale al rischio." />
                <FeatureCard title="2. Ricevi il tuo modello" text="Uno dei 6 portafogli finali ti viene assegnato automaticamente." />
                <FeatureCard title="3. Parti in modo concreto" text="Ti guidiamo su PAC, strumenti e ribilanciamento annuale." />
              </div>

              <div className="mt-8 rounded-2xl border border-slate-200 bg-slate-50 p-5">
                <p className="text-sm text-slate-600">
                  Non aspettare di sentirti pronto. Inizia a costruire il tuo futuro con un piano semplice, coerente e sostenibile.
                </p>
              </div>

              <div className="mt-8 rounded-3xl border border-slate-200 bg-slate-50 p-6">
                <h3 className="text-xl font-semibold">Scopri cosa puo diventare un investimento mensile</h3>
                <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
                  Un piccolo importo investito con continuita puo fare molta piu strada di quanto sembri.
                  Qui sotto puoi fare una simulazione rapida.
                </p>
        ...
        {step === "preview" && (
          <section className="mx-auto max-w-5xl space-y-6">
            <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
              <p className="text-sm font-medium uppercase tracking-[0.2em] text-slate-500">Anteprima risultato</p>
              ...
              <div className="mt-8 rounded-2xl bg-slate-50 p-5">
                <h3 className="text-sm font-semibold uppercase tracking-wide text-slate-600">
                  Cosa sblocchi con il piano completo
                </h3>
                <ul className="mt-3 space-y-2 text-sm text-slate-700">
                  <li>• Modello educativo con composizione e logica</li>
                  <li>• Guida PAC educativa passo passo</li>
                  <li>• Dati storici e cosa evitare</li>
                  <li>• Esempi di strumenti e categorie, solo a fini informativi</li>
                </ul>
              </div>
        ...
        {step === "paywall" && (
          <section className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-[1.2fr_0.8fr]">
            <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
              <h2 className="text-3xl font-bold tracking-tight">Sblocca il tuo modello completo</h2>
              <p className="mt-3 text-slate-600">
                Stai per ricevere un modello assegnato in base alle tue risposte, non una scelta generica.
                Il prodotto include guida operativa, foglio strumenti, spiegazioni semplici e supporto al ribilanciamento.
              </p>

              <div className="mt-6 grid gap-4 md:grid-cols-2">
                <FeatureCard
                  title="Acquisto iniziale"
                  text="29 EUR una tantum per questionario, modello, guida PAC e foglio strumenti."
                />
                <FeatureCard
                  title="Ribilanciamento annuale"
                  text="29 EUR / anno per mantenere il modello coerente nel tempo."
                />
              </div>
              ...
              <div className="mt-6 rounded-2xl bg-slate-50 p-5">
                ...
                />
                <p className="mt-2 text-xs text-slate-500">
                  In questa demo il pagamento e simulato. In produzione qui si colleghera Stripe o un sistema reale di pagamento.
                </p>
              </div>
            ...
            <div className="rounded-3xl border border-slate-200 bg-slate-900 p-8 text-white shadow-sm">
              <p className="text-sm uppercase tracking-[0.2em] text-slate-300">Riepilogo</p>
              <h3 className="mt-3 text-2xl font-bold">{scoreResult.portfolio.title}</h3>
              <p className="mt-3 text-slate-300">{scoreResult.portfolio.whyItFits}</p>

              <div className="mt-8 space-y-3">
                <PriceLine label="Piano iniziale" value="29 EUR" />
                <PriceLine label="Ribilanciamento annuale" value="29 EUR / anno" />
              </div>

              <div className="mt-8 rounded-2xl bg-white/10 p-4">
                <p className="text-sm text-slate-200">
                  Quello che vendiamo e la semplicita: un sistema guidato che aiuta a partire e a continuare con metodo.
                </p>
              </div>
            </div>
          </section>
        ...
        {step === "portfolio" && (
          <section className="space-y-6">
            <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                ...
                <div className="w-full max-w-sm rounded-3xl border border-slate-200 bg-slate-50 p-6">
                  <h3 className="text-sm font-semibold uppercase tracking-wide text-slate-600">Composizione</h3>
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              <h3 className="text-xl font-semibold">Checklist operativa</h3>
              <p className="mt-2 max-w-3xl text-sm leading-7 text-slate-600">
                Completa prima il setup iniziale. Sono i passaggi che rendono operativo il sistema.
              </p>
          ...
          <section className="mx-auto max-w-5xl space-y-6">
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <button
                  onClick={() => onboardingStep === 0 ? setStep("dashboard") : setOnboardingStep((prev) => prev - 1)}
        ...
        {step === "dashboard" && (
          <section className="space-y-6">
            ...
            <div className={`relative overflow-hidden rounded-3xl border p-6 shadow-sm transition-all duration-700 ${
              currentMonthCompleted
            ...
            <div className="grid gap-6 xl:grid-cols-[0.95fr_1.2fr_1fr]">
              ...
              <div className="space-y-6">
                <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="text-xl font-semibold">Ripartizione attuale</h3>
                      <p className="mt-1 text-sm text-slate-600">
                ...
                <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                  <h3 className="text-xl font-semibold">I tuoi investimenti</h3>
                  {holdings.length === 0 ? (
              ...
              <div className="space-y-6">
                <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="text-xl font-semibold">Esempio PAC mensile</h3>
                      <p className="mt-1 text-sm text-slate-600">
